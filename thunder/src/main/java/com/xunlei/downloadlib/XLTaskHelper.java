...
public class XLTaskHelper {
    ...
    public synchronized long addThunderTask(String url, String savePath, String fileName) {
        ...
        if (url.startsWith("ftp://") || url.startsWith("http://") || url.startsWith("https://")) {
            ...
            try {
                return addMagentTask(url, savePath, fileName);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
...
//        return currentFileInfo;
    ...
    public synchronized UrlQuickInfo getUrlQuickInfo(long taskId) {
        UrlQuickInfo urlQuickInfo = new UrlQuickInfo();
        XLDownloadManager.getInstance().getUrlQuickInfo(taskId, urlQuickInfo);
        return urlQuickInfo;
    }
